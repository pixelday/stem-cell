...
Note that this is a draft document, as the library is a "pre-alpha" work in progress.
...
One of the core tenets of this project is that front-end web code (HTML in particular) is the "tip of the spear" of the web. It is the layer that is evaluated and interpreted by search crawlers, bots, validators, audits, etc. But more importantly, it is what is directly presented to the user, so getting it right is critical.

While HTML and CSS are straightforward technologies for engineering teams to understand, they are both difficult to master and are therefore often neglected.

We are aiming for the output of this framework to be **WCAG 2.2 Level AAA conformant out of the box** — with one important, honest caveat. W3C/WAI itself notes that AAA is *not* recommended as a blanket requirement for entire sites, because some AAA success criteria simply cannot be satisfied for all types of content. So we treat AAA as a **shared responsibility**:

- **Stem Cell guarantees** the criteria a framework can control through its defaults and components — e.g., 2.1.3 Keyboard (No Exception), 2.2.3 No Timing, 2.3.2 Three Flashes, and 2.4.10 Section Headings.
- **Stem Cell enables** the remaining, content-dependent criteria — e.g., 1.4.6 Contrast (Enhanced), 1.4.8 Visual Presentation, 1.4.9 Images of Text, 3.1.5 Reading Level, 2.4.9 Link Purpose (Link Only), the time-based-media criteria, and 3.3.6 Error Prevention (All) — so they're achievable by the author rather than fought against.
...
The following color family text and accent pairings driven by the "Background color" each pass AAA contrast ratio:

| Mode | Background color | Role | Text color | Text contrast | Accent color | Accent contrast | AAA target |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Light | `#FFFFFF` | Normal text | `#595959` | 7:1 | `#0037FF` | 7.02:1 | ≥ 7:1 |
| Light | `#FFFFFF` | Large text | `#767676` | 4.54:1 | `#006DFF` | 4.53:1 | ≥ 4.5:1 |
| Dark | `#000000` | Normal text | `#959595` | 7.01:1 | `#0099FF` | 7:1 | ≥ 7:1 |
| Dark | `#000000` | Large text | `#757575` | 4.55:1 | `#006AFF` | 4.5:1 | ≥ 4.5:1 |
...
## Built from first principles; always shippable

The project started as a "hello world" page. It had the absolute bare minimum markup to validate across all tools tested. Starting from the first principles of "minimum code to remain valid and compliant and always shippable," every incremental change is thoroughly tested.

## Current auditing tools

These tools are currently all run manually; there is no continuous-integration (CI) testing yet. Likely CI additions include [HTML Validate](https://html-validate.org/) for markup linting and — for the future TSX/React path — [eslint-plugin-jsx-a11y](https://github.com/jsx-eslint/eslint-plugin-jsx-a11y), plus a headless accessibility engine such as [Pa11y](https://pa11y.org/) (`pa11y-ci`), [axe-core](https://github.com/dequelabs/axe-core) driven through Playwright or Cypress, and [Lighthouse CI](https://github.com/GoogleChrome/lighthouse-ci). All of these can run against the built static HTML, so the absence of a build system isn't a blocker. They would complement automatable versions of the manual tools below:
...
Side note: Mutually exclusive recommendations are yet to be observed on this project (assuming they are very rare), but if they were to occur, accessibility should win, and W3C-developed tools carry more authority.
...
We definitely test the bolded browsers listed on each platform, but are unlikely to test Chrome and Brave and Opera on the same system, since they are unlikely to present meaningful rendering differences.
...
We consider 100% validation with zero issues and warnings to be the *baseline*. Beyond that, we work through established external checklists for the things automated audits don't cover:
...
We don't yet own hardware input devices built specifically for users who require them, but keyboard-only navigation is a hard gate: nothing ships unless the entire page can be operated without ever touching the mouse. (Keyboard access isn't a stand-in for "motor impairment" specifically — it underpins the experience for screen-reader users, switch and voice-control users, and keyboard power users alike.) Our keystroke coverage: Tab, Shift+Tab, Enter, Space, Escape, the arrow keys (Up, Down, Left, Right), Home, End, Page Up, and Page Down. The arrow keys plus Home/End matter specifically for composite ARIA widgets (menus, tabs, listboxes, grids, etc.), as described in the [ARIA Authoring Practices Guide](https://www.w3.org/WAI/ARIA/apg/).
...
Lighthouse gives us lab performance scores, but field data matters too. We also check [PageSpeed Insights](https://pagespeed.web.dev/) and [WebPageTest](https://www.webpagetest.org/), and track Core Web Vitals — Largest Contentful Paint, Interaction to Next Paint (INP, which replaced First Input Delay in 2024), and Cumulative Layout Shift — via the Chrome UX Report where real-user data is available. A minimal-JS, no-build framework should have a natural advantage here, and we treat that as a claim to verify rather than assume.
